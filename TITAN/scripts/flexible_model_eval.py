...
def load_data(
    ...
    logger.info(f'Test dataset has {len(test_dataset)} samples.')
    return test_loader, params, smiles_language, protein_language


def load_model(model_type, model_path, params, smiles_language, protein_language):
    device = get_device()
    logger = logging.getLogger()
...
def test_model(model, test_loader, model_path, save_name):
    device = get_device()
    logger = logging.getLogger()
...
if __name__ == '__main__':
    ...
    # run the training
    test_loader, params, smiles_language, protein_language = load_data(
        args.test_affinity_filepath, args.receptor_filepath, args.ligand_filepath, args.model_path)
    model = load_model(
        args.model_type, args.model_path, params, smiles_language, protein_language)
    test_model(model, test_loader, args.model_path, args.save_name)
