                ...
                if split_channels:
                    for method in pdb_dict:
                        pdb_dict[method] = self.split_feature_attribution_matrix(
                            ...
                            w=len(cdr3),
                            h=len(ep),
                            variability_per_position=variability_per_position,
                            cdr3=cdr3
                        )
            ...
            for pdb_id, methods in attributions.items():
                ...
                for method, attribution in methods.items():
                    if correlation_method == 'pearson':
                        corr_pdb[method], p = correlation_nan(
                            f=pearsonr,
                            x=dist.flatten(),
                            y=attribution.flatten(),
                            with_p=True
                        )
                ...
                self.logger.info(f'{pdb_id} done')
            p_value_stats(model_name=self.name, method_p=method_p)
            return corr_dict
        ...
        def random_correlation():
            ...
            for pdb_id, dist_m in distances.items():
                ...
                for i in range(1000):
                    ...
                    if correlation_method == 'pearson':
                        random_corr.append(correlation_nan(
                            f=pearsonr,
                            x=dist_m.flatten(),
                            y=random_m.flatten()
                        ))
                ...
                for method, attribution in methods.items():
                    ...
                    if correlation_method == 'pearson':
                        correlation_dict[pdb_id][method] = (
                            correlation_nan(
                                f=pearsonr,
                                x=ep_aa_dist.flatten(),
                                y=ep_attribution.flatten()
                            ),
                            correlation_nan(
                                f=pearsonr,
                                x=cdr3_aa_dist.flatten(),
                                y=cdr3_attribution.flatten()
                            )
                        )
                    elif correlation_method == 'spearman':
                        correlation_dict[pdb_id][method] = (
        ...
        def aa_random_correlation():
            ...
            for pdb_id, aa_dist in aa_distances.items():
                for i in range(1000):
                    ...
                    random_a = np.random.rand(len(aa_dist))
                    if correlation_method == 'pearson':
                        correlation.append(correlation_nan(
                            f=pearsonr,
                            x=aa_dist.flatten(),
                            y=random_a.flatten()
                        ))
        ...
        def aa_random_correlation_ps():
            ...
            for pdb_id, aa_dist in aa_distances.items():
                ...
                for i in range(1000):
                    ...
                    aa_dist_cdr3 = aa_dist[ep_len:]
                    if correlation_method == 'pearson':
                        correlation_ep.append(correlation_nan(
                            f=pearsonr,
                            x=aa_dist_ep.flatten(),
                            y=random_a_ep.flatten()
                        ))
                        correlation_cdr3.append(correlation_nan(
                            f=pearsonr,
                            x=aa_dist_cdr3.flatten(),
                            y=random_a_cdr3.flatten()
                        ))
                    elif correlation_method == 'spearman':
                        correlation_ep.append(correlation_nan(
                            f=spearmanr,
